...
class ABCEnv(gym.Env):
    ...
    def _reward(self, u, obs):
        PO = obs[:3] - self.circle[0] # relative vector of drone to circle center
        n = np.cross(self.B - self.A, self.C - self.A) # normal vector to circle
        ...
        c1 = np.cross(self.A - self.circle[0], self.B - self.circle[0]) # correct direction
