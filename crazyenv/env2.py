...
class StaticObstEnv(gym.Env):
    ...
    def step(self, u):
        # reward has to be based on previous action
        reward = self._reward(self._obs(), u)
        ...
        obs = self._obs()
        return obs, reward, self.sim.t > 100 or self.pfc.hittingObstacles(obs[:3]), {}
    ...
    def _obs(self):
        ...
        if np.linalg.norm(grad) > np.linalg.norm(self.action_space.high):  # clip the gradient
            grad *= np.linalg.norm(self.action_space.high) / \
                np.linalg.norm(grad)
